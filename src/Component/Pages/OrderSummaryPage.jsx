...
export default function OrderSummaryPage() {
  ...
  // 1. Calculate Subtotal (Using Regex to handle currency symbols/commas)
  const subtotal = cart.reduce((acc, item) => {
    const numericPrice = parseFloat(item.price.replace(/[^\d.]/g, ""));
    return acc + (numericPrice || 0) * (item.quantity || 1);
  }, 0);

  // 2. Define Fixed Fees (You can change these amounts)
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#FF7A18]"
            />
            <Button className="bg-[#FF7A18] text-white px-6 py-2 rounded-md hover:bg-[#e66a15]">
              Login{" "}
            </Button>
          </div>
        </div>
