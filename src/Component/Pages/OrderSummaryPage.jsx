...
import { orderSummarySchema } from "../Validations/Validations";
...
export default function OrderSummaryPage() {
  ...
  const handleContinue = async (e) => {
    e.preventDefault();
    try {
      await orderSummarySchema.validate(formData, { abortEarly: false });
      setErrors({});
    ...
    <div className="w-full min-h-screen bg-[#F3F4F6] font-Inter py-6 md:py-10 px-4">
      <div className="max-w-3xl mx-auto bg-white py-8 md:py-10 px-4 md:px-12 rounded-2xl shadow-sm border border-gray-100">
        ...
        <form onSubmit={handleContinue}>
          ...
          <div className="mt-8">
            ...
            <textarea
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  instructionForRestaurant: e.target.value,
                })
              }
            />
            {errors.instructionForRestaurant && (
              <p className="text-red-500 text-sm mt-1">
                {errors.instructionForRestaurant}
              </p>
            )}
          ...
          <Button
            type="submit"
            className="bg-[#FF7A18] text-white w-full py-4 rounded-xl font-extrabold text-lg hover:bg-[#e66a15] transition-all transform hover:scale-[1.01] active:scale-[0.99] shadow-lg shadow-orange-200 mt-10"
          >
            Proceed to Checkout
          </Button>
        </form>
